...
class WeatherService {
  ...
  // 내일 부터 모레까지 날씨 목록
  static Future<List<Map<String, dynamic>>> getNextDayWeathers(
      latitude, longitude) async {
    print("next weather list on !");
    List<Map<String, dynamic>> nextWeatherInstancesList = [];
    Map<String, dynamic> nextWeatherInstances = {};

    // 오늘 날짜 조회
    var today = DateUtility().getToday();
    var fcstTime = DateUtility().getTimeForFcst();
...
// 날씨 api call
    ...
    if (response.statusCode == 200) {
      ...
      for (var weather in weatherList.item) {
        ...
        if (nowTime != weather.fcstTime && nowTime != "") {
          nextWeatherInstancesList.add(nextWeatherInstances);
          nextWeatherInstances = {};
        }

        nowTime = weather.fcstTime;
        nextWeatherInstances['DATE'] = weather.fcstDate;
        nextWeatherInstances['TIME'] = weather.fcstTime;

        // 카테고리 별로 분리 해서 데이터 저장
        if (weather.category == "TMP") {
          // 기온
          nextWeatherInstances['TMP'] = "${weather.fcstValue}℃";
        } else if (weather.category == "WSD") {
          // 풍속
          nextWeatherInstances['WSD'] = "${weather.fcstValue}m/s";
        } else if (weather.category == "SKY") {
          // 하늘상태
          // 1 맑음
          // 3 구름많음
          // 4 흐림
          nextWeatherInstances['SKY'] = weather.fcstValue;
        } else if (weather.category == "PTY") {
          // 강수형태
          /*
            (초단기) 없음(0), 비(1), 비/눈(2), 눈(3), 빗방울(5), 빗방울눈날림(6), 눈날림(7) 
            (단기) 없음(0), 비(1), 비/눈(2), 눈(3), 소나기(4) 
          */
          nextWeatherInstances['PTY'] = weather.fcstValue;
        } else if (weather.category == "POP") {
          // 강수확률
          nextWeatherInstances['POP'] = "${weather.fcstValue}%";
        } else if (weather.category == "PCP") {
          // 1시간 강수량
          nextWeatherInstances['PCP'] = weather.fcstValue;
        } else if (weather.category == "REH") {
          // 습도
          nextWeatherInstances['REH'] = "${weather.fcstValue}%";
        } else if (weather.category == "SNO") {
          // 1시간 신적설
          nextWeatherInstances['SNO'] = weather.fcstValue;
        } else if (weather.category == "WSD") {
          // 풍속
          nextWeatherInstances['WSD'] = "${weather.fcstValue}m/s";
        } else if (weather.category == "TMN") {
          // 최저기온
          nextWeatherInstances['TMN'] = "${weather.fcstValue}℃";
        } else if (weather.category == "TMX") {
          // 최고기온
